...
exports.getPens = async (req, res) =>{
    ...
    const { searchField = "pen_id", searchDirection = "ASC" } = req.query;

    // Getter- retrieving all the pens

    const QUERY = "SELECT * FROM pens";
...
// POST function
    ...
    exports.addPen = async (req, res)=>{
        ...
        const fullQuery = `INSERT INTO pens (${columns}) VALUES (${values
            .map((v, i) => `$${i + 1}`)
            .join(", ")}) RETURNING *`;
...
// UPDATE FUNCTION 
    ...
    const { pen_id: pen_id } = req.params;
    ...
    console.log(setStr);
    const query = `UPDATE pens SET ${setStr.slice(
        0,
      setStr.length - 2 // get rid of last ', '
    )} WHERE pen_id = ${pen_id} RETURNING *`;
    ...
    exports.removePen = async(req, res) =>{
